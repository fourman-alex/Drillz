...
//ignore_for_file: avoid_redundant_argument_values, lines_longer_than_80_chars, always_put_required_named_parameters_first, avoid_types_on_closure_parameters, prefer_asserts_with_message
...
class OpenContainer<T extends Object?> extends StatefulWidget {
  ...
  /// Optional size of the open state. If provided the animation will not open
  /// into the enclosing [Navigator]'s size, and instead will open into the
  /// provided size
  final Size? openSize;
...
class _OpenContainerRoute<T> extends ModalRoute<T> {
  ...
  void _takeMeasurements({
    ...
    final Size dialogSize = openSize ?? navSize;
    if (openSize != null) {
      _rectTween.end = Offset((navSize.width - (openSize?.width ?? 0)) / 2,
              (navSize.height - (openSize?.height ?? 0)) / 2) &
          dialogSize;
    } else {
      ...
      child: AnimatedBuilder(
        ...
        builder: (BuildContext context, Widget? child) {
          if (animation.isCompleted) {
            return SizedBox(
              height: openSize?.height ?? double.infinity,
              width: openSize?.width ?? double.infinity,
              child: Material(
                color: openColor,
          ...
          return SizedBox.expand(
            child: Container(
              color: opaque ? scrimTween!.evaluate(curvedAnimation) : null,
              child: Align(
  ...
  @override
  Color? get barrierColor => openSize != null ? Colors.black54 : null;

  @override
  bool get opaque => openSize == null;

  @override
  bool get barrierDismissible => openSize != null;
